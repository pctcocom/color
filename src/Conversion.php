...
class Conversion{
   ...
   public static function rgb2cmyk ($input){
   ...
   public static function cmyk2rgb ($input){
   ...
   public static function rgb2hex ($input){
   ...
   public static function rgb2yuv ($input){
      $r = intval($input[0], 10);
      $g = intval($input[1], 10);
      $b = intval($input[2], 10);
   ...
   public static function yuv2rgb ($input){
      $y = intval($input[0], 10);
   ...
   public static function rgb2hsv ($input){
   ...
   public static function hsv2rgb ($input){
      $h = $input[0];
      $s = $input[1];
      $v = $input[2];
      $i = floor($h * 6);
   ...
   public static function parseColor ($input){
      ...
      if ($geregext !== null) {
         switch ($geregext[1]) {
            case '#':
               return Conversion::colorconv($geregext[2], 'hex2rgb');
            case 'rgb':
               return [intval(trim($geregext[4]), 10), intval(trim($geregext[5]), 10), intval(trim($geregext[6]), 10)];
            case 'hsl':
               return Conversion::colorconv([intval(trim($geregext[4]), 10), intval(trim($geregext[5]), 10), intval(trim($geregext[6]), 10)], 'hsl2rgb');
            case 'yuv':
               return Conversion::colorconv([intval(trim($geregext[4]), 10), intval(trim($geregext[5]), 10), intval(trim($geregext[6]), 10)], 'yuv2rgb');
            default:
               return false;
         }
      }
   ...
   public static function colorconv ($input, $mode='parse', $html=false){
      ...
      switch ($mode) {
      case 'rgb2hsl':
         $output = Conversion::rgb2hsl($input);
         if($html) {
             $output = 'hsl('.$output[0].','.$output[1].'%,'.$output[2].'%)';
         }
         break;
      case 'hsl2rgb':
         $output = Conversion::hsl2rgb($input);
         if($html) {
             $output = 'rgb('.$output[0].','.$output[1].','.$output[2].')';
         }
         break;
      case 'rgb2cmyk':
         $output = Conversion::rgb2cmyk($input);
         if($html) {
             $output = 'cmyk('.$output[0].','.$output[1].','.$output[2].')';
         }
         break;
      case 'cmyk2rgb':
         $output = Conversion::cmyk2rgb($input);
         if($html) {
             $output = 'rgb('.$output[0].','.$output[1].','.$output[2].')';
         }
         break;
      case 'hex2rgb':
         $output = Conversion::hex2rgb($input);
         if($html) {
             $output = 'rgb('.$output[0].','.$output[1].','.$output[2].')';
         }
         break;
      case 'rgb2hex':
         $output = Conversion::rgb2hex($input);
         if($html) {
             $output = '#'.$output;
         }
         break;
      case 'rgb2yuv':
         $output = Conversion::rgb2yuv($input);
         if($html) {
             $output = 'yuv('.$output[0].','.$output[1].','.$output[2].')';
         }
         break;
      case 'yuv2rgb':
         $output = Conversion::yuv2rgb($input);
         if($html) {
             $output = 'rgb('.$output[0].','.$output[1].','.$output[2].')';
         }
         break;
      case 'rgb2hsv':
         $output = Conversion::rgb2hsv($input);
         if($html) {
             $output = 'hsv('.$output[0].','.$output[1].','.$output[2].')';
         }
         break;
      case 'hsv2rgb':
         $output = Conversion::hsv2rgb($input);
         if($html) {
             $output = 'rgb('.$output[0].','.$output[1].','.$output[2].')';
         }
         break;
      case 'hsl2hex':
         $output = Conversion::rgb2hex(hsl2rgb($input));
         if($html) {
             $output = '#'.$output;
         }
         break;
      case 'hex2hsl':
         $output = Conversion::rgb2hsl(hex2rgb($input));
         if($html) {
             $output = 'hsl('.$output[0].','.$output[1].'%,'.$output[2].'%)';
         }
         break;
      case 'complexity2int':
         $output = Conversion::complexity2int($input);
         if($html) {
             $output = 'hsl('.$output[0].','.$output[1].'%,'.$output[2].'%)';
         }
         break;
      case 'int2rgb':
         $output = Conversion::int2rgb($input);
         if($html) {
             $output = 'hsl('.$output[0].','.$output[1].'%,'.$output[2].'%)';
         }
         break;
      case 'complexity2rgb':
         $output = Conversion::colorconv(colorconv($input, 'complexity2int'), 'int2rgb');
         break;
      case 'mixrgb':
         ...
         break;
      case 'parse':
         $output = Conversion::parseColor($input);
         if($html) {
             $output = 'rgb('.$output[0].','.$output[1].','.$output[2].')';
         }
         break;
      default:
         $output = Conversion::colorconv($input, 'parse');
         if($html) {
             $output = 'rgb('.$output[0].','.$output[1].','.$output[2].')';
         }
         break;
